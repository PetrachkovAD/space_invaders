import { WIDTH_CANVAS, HEIGHT_ENEMY_SPRITE, MAX_WIDTH_ENEMY_SPRITE,
  ENEMY_BULLET_SPRITERS, SOUND_ENEMY } from './config';
import GameObject from './gameObject';
import Bullet from './bullet';
// Класс вражеского корабля
// score - очки за корабль
class Enemy extends GameObject {
  constructor (ctx, spriters, x = 0, y = 0, score = 10, dx = -4, dy = HEIGHT_ENEMY_SPRITE) {
...
class EnemyLine extends GameObject {
  constructor (ctx, spriters, count, y = 0, score = 10, space = 10) {
    ...
    this.count = count;
    this.space = space + MAX_WIDTH_ENEMY_SPRITE - spriters[0].w;
...
class EnemyFleet extends GameObject {
  constructor (ctx, lines, y = 0, dx = -4, dy = HEIGHT_ENEMY_SPRITE, space = 8) {
    super(ctx, [], 0, y, dx, dy);
    this.lines = lines;
    this.space = space;
    ...
    SOUND_ENEMY.forEach(function (sound, i) {
      let audio = new Audio();
      audio.src = sound;
  ...
  static makeLines (ctx, enemySpriters, count = 1, size = 10) {
    ...
    let hSprite = HEIGHT_ENEMY_SPRITE;
